"""Answer extraction with multi-tier fallback for flexible format handling.

Adapted from test.py - removes format dependency so models get credit
for correct answers regardless of output format (boxed, "the answer is",
or just the last number in the response).
"""
...
def extract_answer(pred_str, data_name="math", use_last_number=True):
    """Extract answer with multi-tier fallback.
